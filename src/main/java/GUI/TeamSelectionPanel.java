...
public class TeamSelectionPanel extends JPanel {
    ...
    public TeamSelectionPanel(Connection pokedexConn, Connection usuariosConn, JFrame parentFrame, String username) {
    ...
    private void loadAvailablePokemon() {
        ...
        String sql = "SELECT * FROM pokedex ORDER BY ID LIMIT 151";
        ...
        try (PreparedStatement ps = pokedexConn.prepareStatement(sql);
            ...
            while (rs.next()) {
                Pokemon pokemon = new Pokemon(
                    ...
                    rs.getString("Type1"),
                    rs.getString("Type2"),
                    rs.getInt("HP") + rs.getInt("Attack") + rs.getInt("Defense") +
                        rs.getInt("SpAtk") + rs.getInt("SpDef") + rs.getInt("Speed"),
                    rs.getInt("HP"),
                    rs.getInt("Attack"),
                    rs.getInt("Defense"),
                    rs.getInt("SpAtk"),
    ...
    private void startBattle() {
        ...
        parentFrame.setContentPane(new EnhancedBattlePanel(
            selectedTeam, enemyTeam, pokedexConn, usuariosConn, username, parentFrame
        ));
    ...
    private List<Pokemon> generateEnemyTeam() {
        ...
        List<Pokemon> pool = new ArrayList<>(availablePokemon);

        pool.removeAll(selectedTeam);

        java.util.Random rand = new java.util.Random();
        for (int i = 0; i < 5 && !pool.isEmpty(); i++) {
            int index = rand.nextInt(pool.size());
            enemyTeam.add(pool.remove(index));
        }
